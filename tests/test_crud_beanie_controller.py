...
class TestUserBeanieController:
    """Tests del Controller con BeanieBaseController."""

    @pytest.mark.asyncio
    async def test_list_users_pagination(self, init_db, service, sample_users):
        """Test: Listar usuarios con paginación desde controller."""
        from fastapi import Request
        from example_crud_beanie.controller import ListBeanieUsers

        # Mock del request
        request = MagicMock(spec=Request)
        request.scope = {"endpoint": ListBeanieUsers.__call__}

        # Crear instancia del controller
        controller = ListBeanieUsers()
        controller.service = service
        controller.request = request
        controller.action = "__call__"

        # Simular llamada con parámetros
        with patch("inspect.currentframe") as mock_frame:
            # Simular el frame stack
            ...
            mock_frame.return_value.f_back.f_back = caller_frame
    ...
    @pytest.mark.asyncio
    async def test_retrieve_user(self, init_db, service, sample_users):
        """Test: Obtener un usuario desde controller."""
        from fastapi import Request
        from example_crud_beanie.controller import GetBeanieUser

        request = MagicMock(spec=Request)
        request.scope = {"endpoint": GetBeanieUser.__call__}

        controller = GetBeanieUser()
        controller.service = service
        controller.request = request
        controller.action = "__call__"

        user_id = str(sample_users[0].id)
        response = await controller.retrieve(user_id)

        assert response.status == "success"
        assert response.data.name == "Juan Pérez"

    @pytest.mark.asyncio
    async def test_create_user(self, init_db, service):
        """Test: Crear usuario desde controller."""
        from fastapi import Request
        from example_crud_beanie.controller import CreateBeanieUser

        request = MagicMock(spec=Request)
        request.scope = {"endpoint": CreateBeanieUser.__call__}

        controller = CreateBeanieUser()
        controller.service = service
        controller.request = request
        controller.action = "__call__"

        user_data = UserBeanieCreateSchema(
            name="Nuevo Usuario", email="nuevo@example.com", age=28
        )

        response = await controller.create(user_data)

        assert response.status == "success"
        assert response.message == "Creado exitosamente"
        assert response.data.email == "nuevo@example.com"

    @pytest.mark.asyncio
    async def test_update_user(self, init_db, service, sample_users):
        """Test: Actualizar usuario desde controller."""
        from fastapi import Request
        from example_crud_beanie.controller import UpdateBeanieUser

        request = MagicMock(spec=Request)
        request.scope = {"endpoint": UpdateBeanieUser.__call__}

        controller = UpdateBeanieUser()
        controller.service = service
        controller.request = request
        controller.action = "__call__"

        user_id = str(sample_users[0].id)
        update_data = UserBeanieUpdateSchema(name="Nombre Actualizado", age=35)

        response = await controller.update(user_id, update_data)

        assert response.status == "success"
        assert response.message == "Actualizado exitosamente"
        assert response.data.name == "Nombre Actualizado"
        assert response.data.age == 35

    @pytest.mark.asyncio
    async def test_delete_user(self, init_db, service, sample_users):
        """Test: Eliminar usuario desde controller."""
        from fastapi import Request
        from example_crud_beanie.controller import DeleteBeanieUser

        request = MagicMock(spec=Request)
        request.scope = {"endpoint": DeleteBeanieUser.__call__}

        controller = DeleteBeanieUser()
        controller.service = service
        controller.request = request
        controller.action = "__call__"

        user_id = str(sample_users[0].id)
        response = await controller.delete(user_id)

        assert response.status == "success"
        assert response.message == "Eliminado exitosamente"
