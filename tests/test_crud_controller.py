...
class TestUserController:
    """Tests del Controller con integración completa."""

    @pytest.mark.asyncio
    async def test_list_users_pagination(self, service, sample_users):
        """Test: Listar usuarios con paginación desde controller."""
        from fastapi import Request
        from example_crud.controller import ListUsers

        # Mock del request
        request = MagicMock(spec=Request)
        request.state.db = service.repository._session
        request.scope = {"endpoint": ListUsers.__call__}
    ...
    @pytest.mark.asyncio
    async def test_list_with_filters(self, service, sample_users):
        """Test: Listar con filtros desde controller."""
        from fastapi import Request
        from example_crud.controller import ListUsers
    ...
    @pytest.mark.asyncio
    async def test_retrieve_user(self, service, sample_users):
        """Test: Obtener un usuario desde controller."""
        from fastapi import Request
        from example_crud.controller import GetUser

        request = MagicMock(spec=Request)
        request.state.db = service.repository._session
        request.scope = {"endpoint": GetUser.__call__}
        ...
        assert response.status == "success"
        assert response.data.id == sample_users[0].id
    ...
    async def test_create_user(self, service):
        ...
        from fastapi import Request
        from example_crud.controller import CreateUser

        request = MagicMock(spec=Request)
        request.state.db = service.repository._session
        request.scope = {"endpoint": CreateUser.__call__}
    ...
    async def test_update_user(self, service, sample_users):
        ...
        from fastapi import Request
        from example_crud.controller import UpdateUser

        request = MagicMock(spec=Request)
        request.state.db = service.repository._session
        request.scope = {"endpoint": UpdateUser.__call__}
        ...
        user_id = str(sample_users[0].id)
        update_data = UserUpdateSchema(name="Nombre Actualizado", age=35)
    ...
    async def test_delete_user(self, service, sample_users):
        ...
        from fastapi import Request
        from example_crud.controller import DeleteUser

        request = MagicMock(spec=Request)
        request.state.db = service.repository._session
        request.scope = {"endpoint": DeleteUser.__call__}
    ...
    @pytest.mark.asyncio
    async def test_list_with_search(self, service, sample_users):
        """Test: Buscar usuarios desde controller."""
        from fastapi import Request
        from example_crud.controller import ListUsers
        ...
        assert response.data[0].name == "María García"
